...
import { providerManager } from './videoProviders/index';
import { generateScript, improveScript, analyzeReferenceVideo } from './geminiService';

dotenv.config();

export const app = express();

// Generous body parsing limits for base64 image data URLs (up to 50MB)
...
// Detailed Health Check: Reports status of server and individual video engines
app.get('/api/health', (_req: Request, res: Response) => {
  const active = providerManager.getActiveProvider();
  const heygenProvider = providerManager.getProvider('heygen');
  const runwayProvider = providerManager.getProvider('runway');
  const veoProvider = providerManager.getProvider('veo');
  ...
  const heygenConfigured = Boolean(heygenProvider?.isConfigured());
  const runwayConfigured = Boolean(runwayProvider?.isConfigured());
  const veoConfigured = Boolean(veoProvider?.isConfigured());
  ...
  res.json({
    ...
    activeProvider: {
      ...
      isMock: active.id === 'mock',
      category: active.category,
    },
    productionReady: heygenConfigured,
  });
});

// List and inspect AI video generation providers with capabilities
...
// Switch active provider with validation: will not activate unconfigured providers
...
app.get('/api/video/status/:jobId', async (req: Request, res: Response) => {
  try {
    ...
    // Detect if this is a mock job or belongs to another provider
    let provider = providerManager.getActiveProvider();
    if (jobId.startsWith('mock_') && provider.id !== 'mock') {
      const mockP = providerManager.getProvider('mock');
      if (mockP) provider = mockP;
    }
